...
inv1: This invariant ensures that each customer's email is unique and the rating of submitted reviews are between 1 and 5. It also verifies that the phone number length is 10 digits only.
inv1_sat: Phone digits for Customer are exactly 10. Email is unique and rating is between 1 and 5
inv1_fail: Phone digits for Customer are greater than 10


inv2: This invariant ensures that venues cannot be booked beyond their capacity. By checking that bookings exist and that the number of guests does not exceed the venue's capacity, the system prevents overbooking.
inv2_sat: NumberOfGuests for EventBooking and Capacity in Venue are equal
inv2_fail: NumberOfGuests for EventBooking are 51, where Capacity in Venue is 50


inv3: This invariant validates that BookingAdmins manage at least one venue, approve more than 0 bookings, and maintain a positive number of approvals.
inv3_sat: NumberOfApprovals by BookingAdmins is set to 1. 2 Venues are being managed and 1 approvedBooking
inv3_fail: NumberOfApprovals by BookingAdmins is set to 0 and there are 0 approvedBookings performed
